...
#pragma unroll
  for (int i = 0; i < 32; ++i) {
    ...
    *(float4 *)(C_local + (i * 4)) =
        make_float4(broadcast_var, broadcast_var, broadcast_var, broadcast_var);
  }
#pragma unroll
    ...
    for (int kp = 0; kp < 2; ++kp) {
      for (int i_6 = 0; i_6 < 4; ++i_6) {
        for (int j_1 = 0; j_1 < 8; ++j_1) {
          {
            *(((float32x4 *)C_local) + ((i_6 * 8) + j_1)) =
                __builtin_amdgcn_mfma_f32_16x16x32_bf16(
                    *(((bfloat16x8_vec *)B_local) + ((j_1 * 2) + kp)),
      ...
      for (int j_3 = 0; j_3 < 8; ++j_3) {
        {
          *(((float32x4 *)C_local) + ((i_8 * 8) + j_3)) =
              __builtin_amdgcn_mfma_f32_16x16x32_bf16(
                  *(((bfloat16x8_vec *)B_local) + ((j_3 * 2) + kp_1)),
...
extern "C" int init() {
  ...
  if (dev_res_gemm_kernel != hipSuccess) {
    snprintf(error_buf, ERROR_BUF_SIZE,
             "Failed to get HIP device for gemm_kernel: %s",
             hipGetErrorString(dev_res_gemm_kernel));
    return -1;
  }
  int max_smem_gemm_kernel = 0;
  hipError_t attr_res_gemm_kernel = hipDeviceGetAttribute(
      &max_smem_gemm_kernel, hipDeviceAttributeMaxSharedMemoryPerBlock,
      device_gemm_kernel);
  if (attr_res_gemm_kernel != hipSuccess || max_smem_gemm_kernel <= 0) {
    snprintf(error_buf, ERROR_BUF_SIZE,
             "Failed to query HIP max shared memory for gemm_kernel: %s",
             hipGetErrorString(attr_res_gemm_kernel));
    return -1;
  }
  if (131072 > max_smem_gemm_kernel) {
    snprintf(error_buf, ERROR_BUF_SIZE,
             "Requested dynamic shared memory %d exceeds device limit %d for "
             "gemm_kernel",
             131072, max_smem_gemm_kernel);
    return -1;
  }
  return 0;
