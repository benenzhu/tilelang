...
// Variant with pre-computed buffer resource descriptor and base pointer.
// rsrc_base_lo = readfirstlane((uint32_t)(uintptr_t)buffer_base_ptr),
// pre-computed once at kernel entry to avoid per-call readfirstlane overhead.
// voffset is computed relative to rsrc base so hardware gets the correct address.
template <int N>
