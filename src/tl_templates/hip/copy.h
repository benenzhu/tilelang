...
// ============================================================================
// Optimized G2S copy with precomputed SRD / voffset / soffset
//
// Usage pattern (generated by codegen):
//   // Before k-loop (once per kernel):
//   auto srd_A = tl::cp_async_make_srd(A);
//   uint32_t voff_A = tl::cp_async_compute_voffset(&A[swizzle_expr], A);
//
//   // Inside loop (per load):
//   uint32_t soff = <wave_uniform_byte_offset>;
//   uint32_t lds  = <readfirstlane(lds_ptr)>;
//   tl::cp_async_gs_v2<16>(lds, srd_A, voff_A, soff);
//
// This eliminates per-load SRD creation (4 readfirstlane) and per-load
// global base_lo extraction (1 readfirstlane), leaving only 1 readfirstlane
// for the LDS base per load.
// ============================================================================

// Create a buffer resource descriptor (SRD) once from a base pointer.
// The SRD is wave-uniform and can be reused across all loads from the same
// buffer.  Internally calls readfirstlane on all 4 dwords.
TL_DEVICE int32x4_t cp_async_make_srd(const void *base_ptr) {
  return make_wave_buffer_resource(base_ptr);
}

// Compute the per-thread voffset (in bytes) relative to the buffer's base.
// swizzled_ptr  = &A[thread_dependent_swizzle_index]  (per-lane)
// base_ptr      = A  (wave-uniform raw base pointer)
// Returns: byte offset from base_ptr for this lane, suitable for the
//          voffset operand of buffer_load ... lds.
TL_DEVICE uint32_t cp_async_compute_voffset(void const *swizzled_ptr,
                                             void const *base_ptr) {
  uint32_t my_lo =
      static_cast<uint32_t>(reinterpret_cast<uintptr_t>(swizzled_ptr));
  uint32_t base_lo =
      static_cast<uint32_t>(reinterpret_cast<uintptr_t>(base_ptr));
  return my_lo - base_lo;
}

// Fast G2S load using precomputed components.
//   lds_base  = wave-uniform LDS byte address (via readfirstlane)
//   srd       = buffer resource descriptor (from cp_async_make_srd)
//   voffset   = per-lane byte offset (from cp_async_compute_voffset)
//   soffset   = wave-uniform scalar byte offset (changes per tile/iteration)
template <int N>
TL_DEVICE void cp_async_gs_v2(uint32_t lds_base, int32x4_t srd,
                               uint32_t voffset, uint32_t soffset) {
  static_assert(N == 16, "cp_async_gs_v2 only supports N=16");
  ...
  llvm_amdgcn_raw_buffer_load_lds(
      ...
      N,                 // bytes per lane (16)
      voffset,
      soffset,           // wave-uniform scalar offset
      0,                 // immediate offset
      0                  // aux / cache policy
  );
}
